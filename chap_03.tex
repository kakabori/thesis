...
\hline
\end{tabular}
  \caption{Form factors for $h$ = 1 to 4 predicted by different fits.
  In consensus phase column, * indicates an extinct peak, and ? is assigned
  if a consistent phase could not be obtained throughout
  different fits.}
...
\hline
\end{tabular}
\caption{Form factors for $h$ = 5 to 9 predicted by different fits.
  In consensus phase column, * indicates an extinct peak, and ? is assigned
  if a consistent phase could not be obtained throughout
  different fits.}
...
\begin{figure}[htbp]
  ...
  {Two dimensional map of the contour part of the form factor $|\FC(\mathbf{q})|$ 
  given by
...
among three cases. The variation in $D_\text{HH}^\text{minor}$ was
larger, and in Fit5c, $D_\text{HH}^\text{minor}$ = 31.8 \AA.
Presence of the terminal methyl trough in the major arm was robust, but
...
could not be determined. Table~\ref{tab:LAXS_summary} summarizes
...
separation of this peak from the stronger one was well visible between 
...
Sec.~\ref{sec:nGIWAXS_ripple_phase} showed two observed peaks that could be
indexed with our thin rod model. The only phase in Table~\ref{tab:ripple_formulas}
that is compatible with two peaks above the equator is $P_{\beta F}^{\zeta=\pi/2}$.
In this phase, the $q_z$ value of (2, 0) peak is twice larger than ($\pm$1, 1),
which is consistent with our estimated peak positions, $q_z$ = 0.2 $\pm$ 0.01 and
0.12 $\pm$ 0.2. Then, correct indexing of our observed peaks is to
assign the stronger peak to (2, 0) and 
