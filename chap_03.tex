...
the chains in the gel phase \cite{Nagle00}. 
...
The other axis in the phase diagram (Fig.~\ref{fig:phase_diagram})
is the hydration, which was shown to strongly influence the monoclinic 
lattice constants \cite{ref:Wack89}. In multilamellar systems, the hydration
level is indicated by the lamellar repeat $D$-spacing, and 
...
could elucidate how the hydration affects the ripple sawtooth shape, but
...
the molecular organization within the bilayers has only been elusive.
...
This idea was supported by a later analysis of the low angle X-ray scattering 
data from an unoriented DMPC sample, which revealed that 
...
\cite{ref:Sun96} while the smaller thickness of the minor arm
was attributed to chain disordering similarly to the fluid phase.
...
\cite{ref:Lenz07}.
Despite many evidences from experiments and simulations, 
the coexistence of different types molecular packing in the major and minor arms 
...
quantitative wide angle scattering data from an oriented sample.
...
like the fluid $L_\alpha$ phase, or interdigitated like in the $L_\beta I$ phase. 
...
To compare the structure obtained from $|F_{hk}^\text{un}|$ to $|F_{hk}^\text{ori}|$,
...
\section{WAXS Results}\label{sec:waxs_results}
...
tWAXS where we were able to carefully measure the width of 
...
Kamal \textit{et al.} developed a Landau-Ginzburg theory that allows 
...
\begin{figure}[htbp]
  ...
  and the minor arm in the ripple phase (right). The left image is taken from 
  \cite{Kucerka05_BPJ}. The ripple minor arm profiles were calculated 
...
While Fig.~\ref{fig:interdigitation_comparison}
...
\begin{figure}[htbp]
  ...
  and the minor arm in the ripple phase (right). The left image is taken from 
  \cite{ref:Guler09}. The ripple minor arm profiles were calculated 
  using the phases predicted by Fit2, 5, and 7 (black, red, and green, respectively). 
  The ripple profiles are scaled to match approximately
...
A further effort should be made to obtain the correct phase factors for more orders
...
Also, given that $h=9$ orders were well observed, it would be possible
to measure even higher orders.
...
tilted with respect to the bilayer normal, toward the ripple direction.
By combining the structural parameters obtained from the LAXS and WAXS studies,
we have determined that chains in the major arm are slightly more 
disordered than all-trans chains.
The Bragg rod indexing suggests that the observed Bragg rods arise 
from the major arm, and not from the interdigitated chains 
in the minor arm hypothesized by a MD simulation.
...
which could be attributed to the fluid like chains in the minor arm.
...
close to the full hydration, it might be possible to observe 
the $P_{\beta I}$ phase of the major arm. This would strongly support
our analysis of the nGIWAXS data,
which suggested existence of the $P_{\beta F}$ phase below the full hydration.
...
should also be done, which would convolute the titled ring patterns from 
titled fluid domains in different directions. It would be interesting to
