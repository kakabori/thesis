...
\cite{ref:Wack88}. By analyzing the X-ray form factor measured by Wack
and Webb \cite{ref:Wack88}, Sun \textit{et al.} \cite{ref:Sun96} was able to
...
(The setup for the low angle X-ray scattering experiment should be 
described in chapter 2 since it is common among both Tat and ripple)
In order to achieve a D-spacing comparable to that of Wack and Webb
($D \approx 57.9$ \AA), 
current was applied to the Peltier, which heated the 
surface on which the sample was situated. 
The temperature gradient caused by this heating Peltier was negligible 
(less than 0.1 \textcelsius).
...
Instead of multilayer monochromator with 1.5\% energy dispersion, 
...
\begin{figure}
  \centering
  \caption{show the beam profile}
  \label{fig:NGIWASX_beam}
\end{figure}
