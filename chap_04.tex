...
Several MD (molecular dynamics) simulations have shed light on molecular 
...
$q_z = 0.12 \textrm{\AA}^{-1}$ as Fig.~\ref{fig:qrplots} shows. 
...
(some thought) Show q swaths for various $\phi$.
...
Because of a lager instrumental resolution than in the NGIWAXS experiment,
the weaker peak was not well separated. Figure~\ref{fig:twaxs_qrplots} shows
a slight hint of the second peak at $q_z=0.12$ \AA$^{-1}$. Indeed, this
...
a sharp peak at $q_z$ = 0 \AA$^{-1}$, which complements the NGIWAXS data.

\begin{figure}
  \centering
  \includegraphics[width=0.5\textwidth]{figures/ripple/TWAXS/twaxs_ripple}
