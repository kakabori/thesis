...
and physical chemists since its discovery (not exactly? may be first structural
study, calorimetry for
alcanes, sturevant, Rand) by Tardieu \textit{et al.} 
...
Get to the figure first, so people know unit cells.
...
(AFM, electron microscopy \cite{} and scanning tunneling microscopy \cite{}) techniques.
...
(Back reference to RR in Tat chapter)
The sample for transmission study was deposited on a thin, 35 micron, silicon
wafer, oriented see sec. Because the wafer was very fragile, attaching the sample to a sticky 
...
\subsection{Geometric Broadening}
WAXS vs LAXS
Make a table (in pixel and q)

\subsection{Low Angle X-ray Scattering Experiment}
(Three different runs, Robin in high resolution, Transmission was in 2011)
...
\begin{figure}[htbp]
  ...
  Each pixel is 0.07113 mm. Given a CCD resolution of at S2 = 359.7 mm.
  beam FWHM = $\Delta\theta$.}
...
Sheets of molybdenum were used to attenuate the beam and the strong orders. 
The attenuation length
$\mu$ of 8 keV X-ray in Mo is 6.433 $\mu$m for and 13.74 $\mu$m for 10.55 keV 
\cite{ref:cxro}.
...
be 6.9 for the wavelength used (1.175 \AA). 100 and 200 $\mu$m for h=1,2
and 200 or 225 for beam.
...
\begin{figure}[jtbp]
  ...
  at a fixed angle of incidence (0.8\textdegree). The sample was an oriented film of 
  ...
  The same gray scale is used in both images. 100 pixel = ? in q A little do
  between the first and second orders is specular.}
  \label{fig:olddopc}
\end{figure}
...
\begin{figure}[htbp]
  ...
  without the attenuator (black solid circles, right). Exposure time?}
...
(edge effect)The width of 1 mm was chosen simply because I could not trim more
...
unresolved width of a intrinsically infinitely narrow wide angle peak if the correlation length was very large.
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.5\textwidth]{figures/ripple/geometric_broadening}
  \caption{The geometric broadening due to the sample width $w_s$ and the beam width $\Delta x_\textrm{beam}$.
  A top view of the sample (gray) on the Si wafer (blue), and 
  ...
  the sample width as $w_\textrm{s}$, the total in-plane scattering
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newpage
\section{LAXS analysis}\label{sec:some_theories}
\subsection{Lattice Structure}\label{sec:lattice_structure}
The unit cell vectors in two-dimensional oblique lattice shown in Fig.~\ref{fig:lattice}
can be expressed as 
\begin{equation}
...
\end{align}
Our sample consists of a uniform distribution of in-plane directions of the
ripple wave vector, $\mathbf{b}$. In this case, $q_{hk}^x$ and $q_{hk}^y$ are combined to 
...
In order to capture all hk peaks in one X-ray exposure, 
the sample was continuously rotated about the x-axis. As a result of this rotation, 
$k=0$ peaks become arcs that subtend an angle $2\theta_{h0}$,
as shown in Fig.~\ref{fig:ewald_main}, with its length
equal to $2\theta_{h0}\qz{h0}$. Cut off fact, say here $2\theta $
...
This is a preview, looking at same h but different k
Because the sample has in-plane rotational symmetry about the $z$-axis, $k\neq 0$
...
\begin{equation}
  I=2\pi \qr{hk} \Io{hk}. \label{eq:side_ewald}
\end{equation}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.5\textwidth]{figures/ripple/absorption_LAXS}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{figures/ripple/abs_factor}
...
As in ref, we take the ripple profile to have a sawtooth profile. Its
...
As shown, we measured scattering on oriented samples in almost identical conditions as the
best unoriented sample of Wack and Webb. 
