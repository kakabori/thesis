...
The instrumental resolution of our X-ray experiments depended on the beam
...
in the 2011 run. The ripple phase experiment in the 2011 run was not successful
...
that the beam was divergent. Then, from similar triangles, we calculated the divergence.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{ccc}
    ...
    experiment & (rad.) & (rad.) \\
    \hline
    LAXS & $4.2 \times 10^{-5}$ & $1.6 \times 10^{-4}$ \\
    NGIWAXS & $4.2 \times 10^{-5}$ & $1.6 \times 10^{-4}$ \\
    TWAXS & $2.5 \times 10^{-5}$ & $5 \times 10^{-5}$ \\
    \hline
  \end{tabular}
...
1.5\% was used in the LAXS and TWAXS experiments. 
...
\begin{table}[htbp]
  ...
  \begin{tabular}{cccc}
    \hline
    type of & $\Delta E/E$ & $E$ & $\lambda$ \\
    experiment & (\%) & (keV) & (\AA) \\
    \hline
    LAXS & 1.5 & 10.55 & 1.175 \\
    NGIWAXS & 0.01 & 10.55 & 1.175 \\
    TWAXS & 1.5 & 10.54 & 1.176 \\
    \hline
  \end{tabular}
...
horizontal beam width. Geometric broadening in the vertical $z$ direction
...
The width of 1 mm was chosen because (1) I could not trim more
...
the resolution was calculated to be $\Delta x = 0.57$ mm = 8 pixels, 
...
\ref{fig:gb_trans1}. When the sample is tilted by $\omega$, rays emerging 
from the top edge of the sample travel extra distance compared to the distance 
that rays from the bottom edge of the sample travel. This, then, leads to 
...
Observed intensity for a reflection with a Bragg angle of $\theta_B$ is
given by
\begin{equation}
