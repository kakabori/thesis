...
\begin{figure}[htbp]
  ...
  fitted to double Lorentzian functions]
...
\section{Interpretation of WAXS}
...
$L$ = 43.5 \AA\.
Tristram-Nagle \textit{et al.}~\cite{Tristram-Nagle02} reported
for the DMPC \LbetaI\ phase
the bilayer thickness $\DHH$ = 40.1 \AA\, chain tilt angle $\theta$ = 
32.3\textdegree, and hydrocarbon thickness $2D_c$ = 30.3 \AA.
These values correspond to the chain length $L_\text{chian}$ = 17.9 \AA.
...
While the positions of the observed Bragg rods are consistent
with a prediction of our thin model, 
integrated intensities are not. Our model shows
that the Bragg rod at smaller $q_z$ is a superposition of (1, 1) and (1, -1) rods,
giving a multiplicity factor of 2 for that rod.
This means that integrated intensity of the weaker Bragg rod should be twice 
that of the stronger one, which is opposite of our results.
...
\begin{figure}[htbp]
  ...
  \caption{Definitions of structural quantities. Chains are tilted with respect
...
\begin{table}[htbp]
  ...
  \begin{tabular}{cccccccc}
    \hline
     & ($m$, $n$) & $q$ & $d_{cc}$ & $q_z$ & $\theta$ & $2D_c$ & $L_\text{chain}$ \\
     & & (\AA$^{-1}$) & (\AA) & (\AA$^{-1}$) & & (\AA) & (\AA) \\
    \hline
    ...
    \hline
    \multirow{2}{*}{$P_{\beta F}^{\zeta=\pi/2}$} & strong & 1.491 & 4.21 & 0.20 & \multirow{2}{*}{19.7\textdegree} & \multirow{2}{*}{31.2} & \multirow{2}{*}{16.5} \\
