...
Cell penetrating peptides (\gls{CPP}) easily penetrate cell membranes 
...
entry into giant
unilamellar vesicles (\gls{GUV}s) composed of phosphatidylserine (\gls{PS}):phosphatidylcholine (\gls{PC})
(1:4 mole ratio) lipids of rhodamine-tagged Tat
is immeasurably slow, but it crosses GUVs composed of 
PS:PC:phosphatidylethanolamine (\gls{PE}) (1:2:1) lipids 
within 30 seconds \cite{Mishra08}. 
This study suggests that negative curvature, induced by the PE, 
facilitates translocation. 
In a subsequent study using much smaller diameter unilamellar vesicles (\gls{LUV}s),
...
of PC:phosphatidylglycerol(\gls{PG}) (3:2) but does translocate across GUVs of the same lipid composition 
...
Thoren \textit{et al.} carried out circular dichroism (\gls{CD}) spectroscopy on a 
...
PC:PG (3:1) vesicles. In addition, solid state \gls{NMR} (nuclear magnetic resonance) has identified a random coil 
...
In this thesis, I combined experimental low-angle X-ray scattering (\gls{LAXS}) 
...
(\gls{LAXS}) as well as fixed angles of incidence $\omega$ for wide angle X-ray 
scattering (\gls{WAXS}).
...
Molybdenum between the sample and the charge-coupled device (\gls{CCD}) detector
...
electron-dense headgroups, providing the head-head spacing $\DHH$, %\gls{DHH},
