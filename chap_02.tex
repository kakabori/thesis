...
The absolute form factor $|F(q_z)|$ is obtained through the relation
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.45\textwidth]{figures/Tat/SDP_Results/XFF/DOPC_XFF1} 
  \includegraphics[width=0.45\textwidth]{figures/Tat/SDP_Results/EDP/DOPC_EDP1} 
  \includegraphics[width=0.45\textwidth]{figures/Tat/SDP_Results/XFF/DOPC_Tat_62to1_3p0_XFF1}
  \includegraphics[width=0.45\textwidth]{figures/Tat/SDP_Results/EDP/DOPC_Tat_62to1_3p0_EDP1} 
  \includegraphics[width=0.45\textwidth]{figures/Tat/SDP_Results/XFF/DOPC_Tat_28to1_3p0_XFF1}
  \includegraphics[width=0.45\textwidth]{figures/Tat/SDP_Results/EDP/DOPC_Tat_28to1_3p0_EDP1}
  \includegraphics[width=0.45\textwidth]{figures/Tat/SDP_Results/XFF/DOPC_Tat_16to1_3p0_XFF1}
  \includegraphics[width=0.45\textwidth]{figures/Tat/SDP_Results/EDP/DOPC_Tat_16to1_3p0_EDP1}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{c|c|ccc|ccc|ccc}
    ...
    \hline
  \end{tabular}
  \caption{Fitting Results for DOPC:DOPE (1:1) membranes for the THG model. $\Delta z_1 = \zPC-\zCG$
...
Fig.~\ref{fig:MD_dopc_70_PC-CG} with the atoms in the simulation parsed
into the same component groups as in the model used in Sec.~\ref{sec:SDP_results}.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.6\textwidth]{figures/Tat/MD_Results/xff/dopc_sim-exp}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.6\textwidth]{figures/Tat/MD_Results/edp/dopc_70_PC-CG}
...
Simulated form factors for DOPC:Tat (128:2, $\xTat$ = 0.015),
where there is one Tat in each monolayer with 64 DOPCs are shown in 
...
factors to the experimental form factor is summarized in 
...
from modeling approach in Sec.~\ref{sec:SDP_results}.
Consistent with the modeling approach, a decrease in the bilayer thickness
and increase in the area per lipid
were indicated from the simulations as Tat concentration increased.
Tat position is found in the headgroup position in both simulations 
and modeling, but $\zTat$ is somewhat larger in simulations.
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.9\textwidth]{figures/Tat/MD_Results/pore}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.9\textwidth]{figures/Tat/MD_Results/DHH_DPP_AL_zTat}
  \caption{}
  \label{fig:DHH_DPP_AL_zTat}
\end{figure}

The average bilayer thickness defined by the average phosphorus-phosphorus 
distance $\langle\Dphos\rangle$ is shown in Table~\ref{tab:local_thickness} 
for simulations with $\zTat$ = 16 and 18 \AA$^2$. 
$\langle\Dphos\rangle$ was measured from a plot of
the electron density profile of phosphorus atoms using
the SIMtoEXP software. 
We also obtained local phosphorus-phosphorus distance $\Dphos$
using the methods described in Sec.~\ref{sec:local_thinning}. 
As the $\Dphos$ column shows, the membrane thickness
was smaller near the Tats compared to the average thickness $\langle\Dphos\rangle$.
Decrease in the membrane thickness $\Delta t = \langle\Dphos^0\rangle - \Dphos$
with respect to the thickness $\langle\Dphos^0\rangle$ of a pure DOPC bilayer
was larger for higher concentrations of Tat.
Assuming that the two leaflets are decoupled, we also estimated the position
of phosphorus atoms $\zphos$ from the bilayer center near the Tats 
...
\begin{table}[htbp]
  ...
  \begin{tabular}{ccccccccc}
    \hline
    \rule{0pt}{14pt} % add extra space  
    $\xTat$ & $\AL$ & $\zTat$ & $\chi^2$ & $\langle\Dphos\rangle$ & $\Dphos$ & $\Delta t$ & $\zphos$ & $\zguan$ \\
    \hline
    0     & 70 &    &      & 36.3 \\
    0.015 & 72 & 18 & 18.0 & 35.6 & 32.8 & 3.5 & 14.7 & 15.5 \\ 
    0.015 & 72 & 16 & 24.9 & 36.1 & 33.0 & 3.3 & 14.9 & 14.5 \\
    0.015 & 74 & 18 & 21.3 & 35.0 & 33.0 & 3.3 & 14.9 & 16.5 \\
    0.015 & 74 & 16 & 25.9 & 35.0 & 32.1 & 4.2 & 14.0 & 13.5 \\
    0.030 & 74 & 18 & 24.3 & 35.3 & 32.6 & 3.7 & 14.5 & 15.5 \\ 
    0.030 & 74 & 16 & 40.1 & 35.3 & 31.2 & 5.1 & 13.1 & 13.5 \\
    0.030 & 76 & 18 & 14.8 & 34.2 & 32.0 & 4.3 & 13.9 & 16.5 \\
    0.030 & 76 & 16 & 30.4 & 34.9 & 31.4 & 4.9 & 13.3 & 14.5 \\
    \hline
  \end{tabular}
  \caption{Local bilayer structural quantities obtained at various
  constrained $\AL$ and $\zTat$.
...
Table~\ref{tab:lateral_decay} shows the lateral decay length of Tat 
perturbation estimated using the method described in Sec.~\ref{sec:lateral_decay}.
The $\HTat$ column gives the values of FWHM of Tat electron density profile.
The $\RTat$ column was calculated by assuming that Tat is a cylinder whose
radius $\RTat$ is calculated from the height $\HTat$ using the experimentally 
measured volume of Tat. A cylindrical shape was chosen to reflect the azimuthal symmetry
of the membrane. We did not consider other rotationally symmetric shapes.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{cccccc}
    ...
    \hline
  \end{tabular}
  \caption{Lateral decay length of Tat perturbation. $R_3$ = 38 \AA.}
  \label{tab:lateral_decay}
\end{table}

The $\chi^2$ obtained by comparison to experiment indicated that the best 
$\zTat$ lay between the simulated values of 16 \AA\ and 18 \AA, and
the best $A_L$ lay between the simulated values of 72 \AA$^2$ and 
74 \AA$^2$, so averages were obtained from these four combinations of $\zTat$ and 
$A_L$, weighted inversely with their $\chi^2$.
Fig.~\ref{fig:figure9} summarizes the results for how Tat affects a DOPC bilayer
based on the weighted average values shown in Table~\ref{tab:MD_summary}.
Tat modeled as a cylinder with height $\HTat$ = 8.7 \AA\ 
and radius $\RTat$ = 8.3 \AA\ is centered at $\zTat$ = 17.1 \AA.
The phosphorus atoms within the perturbed region are positioned at 
$\zphos$ = 14.6 \AA.
...
Fig.~\ref{fig:DHH_DPP} shows that the average 
membrane thickness, as measured by the distance $\DPP$ between phosphocholines 
or the distance $\DHH$ between the maxima in the 
electron density profiles of opposite surfaces, decreases with increasing 
Tat concentration. 
...
hydrocarbon ($\sim$0.3 e/\AA$^3$). Although the thinning shown in Fig.~\ref{fig:DHH_DPP}
is not large, it obviously requires interaction of Tat with the bilayers. 
Fig.~\ref{fig:AL} shows that $\AL$ increases with increasing Tat concentration, by both 
...
bilayer/water interface as indicated in Sec.~\ref{sec:SDP_results}, 
and they are close to 
the phosphocholine headgroup region by comparing the simulated 2$\zTat$
to $\DPP$ in Fig.~\ref{fig:DPP_2zTat}. 
...
(shown in Fig. 4.C) eliminates this spurious result. 
Fig. 7D also shows that modeling gives smaller values for $\zTat$ than the simulation. 
...
samples that had a range of lamellar $D$ spacings (see Fig.~\ref{fig:figure2} caption) that were considerably
larger than the thickness of the bilayer in Fig.~\ref{fig:DHH_DPP}, 
thereby providing an ample pure water space,
typically greater than 20\AA. The result that $2\zTat$ shown in Fig.~\ref{fig:DPP_2zTat} 
is so much smaller than our
...
to experimental X-ray form factors show that Tat contains neither $\beta$ nor $\alpha$-helix structures.
...
in contradiction to our result in Fig.~\ref{fig:DHH_DPP} that shows thinning \cite{Choi12}. 
...
calorimetry and 2H- and 31P-NMR, Ziegler \textit{et al.} \cite{Ziegler03}
...
that these interactions tend to soften the membrane and increase the 
