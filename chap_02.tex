...
\end{equation}
where $i$ specifies a particular molecular component, and
...
\begin{figure}[htbp]
  ...
  and radius $\RTat$. The local bilayer thickness is defined as $D_\textrm{phos-phos}'$. 
...
phosphorus atoms lie within these 9 \AA\ cylinders around a Tat. 
...
9 \AA\ of any Tat position. Then, $z$ positions
of the chosen phosphorus atoms were placed in a list. 
$\zphos$ were calculated from the list. 
...
$\zphos^0$ is half of the average $\Dphos$ 
...
\begin{figure}[htbp]
  ...
  $\langle\zphos\rangle = \langle h(r)\rangle$ is half of the average 
  $\Dphos$ distance.}
...
\begin{align}
  ...
  &  + \frac{1}{2}\left(\zphos R_2-\zphos^0 R_1\right)\left(R_1+R_2\right). 
  \label{eq:r_integ}
\end{align}
...
The result indicates that $\AL$ increases with increasing Tat concentration,
consistent with results from our model fitting approach in Sec.~\ref{sec:SDP_results}.
...
\begin{table}[htbp]
  ...
  constrained $\AL$ and $\zTat$. Units for symbols are \AA\, 
  except for $\AL$ (\AA$^2$).}
